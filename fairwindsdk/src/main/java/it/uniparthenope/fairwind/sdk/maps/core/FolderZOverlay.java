...
public class FolderZOverlay extends Overlay {
    protected TreeSet<FolderZOverlay.ZOverlay> mList;
    protected String mName, mDescription;

    protected class ZOverlay implements Comparator<FolderZOverlay.ZOverlay> {
        float mZIndex;
        ...
        @Override public int compare(FolderZOverlay.ZOverlay o1, FolderZOverlay.ZOverlay o2){
            return (int)Math.signum(o1.mZIndex - o2.mZIndex);
        }

        public float getmZIndex() { return mZIndex; }
    ...
    public FolderZOverlay(){
        super();
        mList = new TreeSet<>(new ZOverlayComp());
        mName = "";
        mDescription = "";
    }
    ...
    public boolean add(Overlay item, float zIndex){
        return mList.add(new FolderZOverlay.ZOverlay(item, zIndex));
    }
    ...
    protected FolderZOverlay.ZOverlay get(Overlay overlay){
        Iterator<FolderZOverlay.ZOverlay> itr = mList.iterator();
        while (itr.hasNext()) {
            FolderZOverlay.ZOverlay item = itr.next();
    ...
    public boolean remove(Overlay overlay) {
        FolderZOverlay.ZOverlay item = get(overlay);
        if (item != null) {
            mList.remove(item);
            return true;
        }
    ...
    protected class ZOverlayComp implements Comparator<FolderZOverlay.ZOverlay> {
        @Override public int compare(FolderZOverlay.ZOverlay o1, FolderZOverlay.ZOverlay o2){
            return (int)Math.signum(o1.mZIndex - o2.mZIndex);
        }
    }
