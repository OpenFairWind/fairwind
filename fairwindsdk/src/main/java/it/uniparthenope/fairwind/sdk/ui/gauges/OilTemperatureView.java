 ...
 */
public class OilTemperatureView extends TemperatureView {
    private String propulsionId;

    public OilTemperatureView(Context context) {
        super(context);
        initializeViews(context,null,0);
    }

    public OilTemperatureView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initializeViews(context,attrs,0);
    }

    public OilTemperatureView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setPropulsionId(String propulsionId) {
        if (propulsionId!=null) {
            ...
            String path = FairWindModelBase.getPropulsionOilTemperaturePath(vessel_uuid, propulsionId);
            setEvent(new FairWindEvent(path + ".*", path, 0, PathEvent.EventType.ADD, this));
        }
    }

    @Override
    protected int getGaugeLayout() { return R.layout.oil_temperature_view; }


    private void initializeViews(Context context, AttributeSet attrs, int defStyle) {
        if (attrs!=null) {

            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.OilTemperatureView);
